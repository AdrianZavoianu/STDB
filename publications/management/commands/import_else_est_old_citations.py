...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--journal", required=True, help="Journal code (e.g., ELSE_EST)")
        ...
        for txt_file in txt_files:
            ...
            for entry in entries:
                try:
                    ...
                    # Extract issue - handle "Issue X", "Issues X–Y", or "Volume X, Issue Y"
                    issue_match = re.search(r"Issue(?:s)?\s+(\d+)", entry, re.I)
                    if not issue_match:
                        issue_match = re.search(r"Volume\s+\d+,\s*Issue(?:s)?\s+(\d+)", entry, re.I)
                    ...
                    for i, line in enumerate(lines):
                        if line.lower().startswith("engineering structures") or "journal" in line.lower():
                            if i >= 2:
                    ...
                    # Get article_index for (journal, volume, issue)
                    existing_max_index = (
                        model.objects
                        .filter(journal=journal_instance, volume=vol, issue=issue)
                        .aggregate(Max("article_index"))
                    )
                    article_index = (existing_max_index["article_index__max"] or 0) + 1
                    ...
                    article.save()
                    self.stdout.write(f"✅ Imported ({vol}-{issue}-{article_index}): {title[:60]}...")
