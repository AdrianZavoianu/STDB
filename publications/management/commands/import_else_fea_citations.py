        ...
        for txt_file in txt_files:
            ...
            for entry in entries:
                try:
                    ...
                    # ✅ Extract issue (optional, handle Issue X or Issues X–Y)
                    ...
                    # ✅ Authors and title from first 2 lines
                    ...
                    # ✅ Always assign unique sequential index per volume (ignore article number)
                    max_index = (
                        model.objects
                        .filter(journal=journal_instance, volume=vol)
                        .aggregate(Max("article_index"))
                    )
                    article_index = (max_index["article_index__max"] or 0) + 1

                    # Final duplicate check by index
                    if model.objects.filter(journal=journal_instance, volume=vol, article_index=article_index).exists():
                        self.stderr.write(f"⚠️ Duplicate index: Vol {vol} Index {article_index} — skipping {doi}")
                        continue
